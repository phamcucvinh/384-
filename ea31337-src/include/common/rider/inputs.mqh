...
input ENUM_STRATEGY Strategy_M30 = STRAT_META_MARTINGALE;  // Strategy on M30 (filter=8)
input ENUM_STRATEGY Strategy_H1 = STRAT_META_WEEKDAY;      // Strategy on H1 (filter=16)
input ENUM_STRATEGY Strategy_H2 = STRAT_STOCHASTIC;        // Strategy on H2 (filter=32)
...
input ENUM_EA_ADV_ACTION EA_Task2_Then = EA_ADV_ACTION_CLOSE_MOST_LOSS;    // 2: Task's action (filter=2)
input ENUM_EA_ADV_COND EA_Task3_If = EA_ADV_COND_NONE;                     // 3: Task's condition (filter=4)
input ENUM_EA_ADV_ACTION EA_Task3_Then = EA_ADV_ACTION_NONE;               // 3: Task's action (filter=4)
